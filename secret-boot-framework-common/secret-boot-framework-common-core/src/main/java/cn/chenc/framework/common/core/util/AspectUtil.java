package cn.chenc.framework.common.core.util;

import cn.chenc.framework.common.core.exception.CommonException;
...
public enum AspectUtil {
    ...
    public String getKey(JoinPoint point, String extra, String prefix) throws NoSuchMethodException {
        ...
        if (null == currentMethod) {
            throw new CommonException("Invalid operation! Method not found.");
        }
    ...
    public String parseParams(Object[] params, String bussinessName) throws JsonProcessingException {
        if (bussinessName.contains("{") && bussinessName.contains("}")) {
            List<String> result = RegexUtils.match(bussinessName, "(?<=\\{)(\\d+)");
            for (String s : result) {
                int index = Integer.parseInt(s);
                bussinessName = bussinessName.replaceAll("\\{" + index + "}", MAPPER.writeValueAsString(params[index - 1]));
            }
        }
